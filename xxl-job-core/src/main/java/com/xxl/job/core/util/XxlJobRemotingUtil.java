...
public class XxlJobRemotingUtil {
    ...
    public static  ReturnT dubboBody(String className,String group,int timeout, String url,String  applicationName){
        ReturnT result=new ReturnT();
        DubboCache dubboCache=new DubboCache();
        logger.info("请求dubbo的url={},className={}",url,className);
        GenericService genericService = dubboCache.getServiceByCache(group, null, timeout, applicationName, className, url);
        if(genericService==null){
            return ReturnT.FAIL;
        }
        //单独执行
        HashMap hashMap=(HashMap) genericService.$invoke("excute",null,null);
        if(hashMap!=null) {
            result.setCode((int) hashMap.get("code"));
            result.setContent(hashMap.get("content"));
            result.setMsg(hashMap.get("msg") + "");
        }
        return result;
    }
